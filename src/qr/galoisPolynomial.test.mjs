import { e, ln, mult256PolyLn, rem256Poly } from './galoisPolynomial.mjs';
...
describe('galois field', () => {
  it('e(ln(x)) is x for 1 <= x <= 255', () => {
    for (let x = 1; x < 256; ++x) {
      expect(e(ln(x))).equals(x);
    }
  });

  it('ln(e(x)) is x for 0 <= x <= 254', () => {
    for (let x = 0; x < 255; ++x) {
      expect(ln(e(x))).equals(x);
    }
  });

  it('e(x) repeats modulo 255', () => {
    for (let x = 0; x < 255; ++x) {
      expect(e(x + 255)).equals(e(x));
    }
  });

  it('e(x) has specific values for 0 <= x <= 254', () => {
    const r = [];
    for (let x = 0; x < 255; ++x) {
      r.push(e(x));
    }
  ...
  it('ln(x) has specific values for 1 <= x <= 255', () => {
    // note: ln(x) is not defined for x = 0, x > 255
    const r = [];
    for (let x = 1; x < 256; ++x) {
      r.push(ln(x));
    }
    expect(r).equals([
      0, 1, 25, 2, 50, 26, 198, 3, 223, 51, 238, 27, 104, 199, 75, 4, 100, 224,
      14, 52, 141, 239, 129, 28, 193, 105, 248, 200, 8, 76, 113, 5, 138, 101,
      47, 225, 36, 15, 33, 53, 147, 142, 218, 240, 18, 130, 69, 29, 181, 194,
      125, 106, 39, 249, 185, 201, 154, 9, 120, 77, 228, 114, 166, 6, 191, 139,
      98, 102, 221, 48, 253, 226, 152, 37, 179, 16, 145, 34, 136, 54, 208, 148,
      206, 143, 150, 219, 189, 241, 210, 19, 92, 131, 56, 70, 64, 30, 66, 182,
      163, 195, 72, 126, 110, 107, 58, 40, 84, 250, 133, 186, 61, 202, 94, 155,
      159, 10, 21, 121, 43, 78, 212, 229, 172, 115, 243, 167, 87, 7, 112, 192,
      247, 140, 128, 99, 13, 103, 74, 222, 237, 49, 197, 254, 24, 227, 165, 153,
      119, 38, 184, 180, 124, 17, 68, 146, 217, 35, 32, 137, 46, 55, 63, 209,
      91, 149, 188, 207, 205, 144, 135, 151, 178, 220, 252, 190, 97, 242, 86,
      211, 171, 20, 42, 93, 158, 132, 60, 57, 83, 71, 109, 65, 162, 31, 45, 67,
  ...
  it('ln(0) is defined as 0 (special case)', () => {
    expect(ln(0)).equals(0);
  });
});
