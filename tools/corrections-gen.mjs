...
const data = `const DATA_L =\n  ${quote(writeCompressed(DATA_L))};
const DATA_M =\n  ${quote(writeCompressed(DATA_M))};
const DATA_Q =\n  ${quote(writeCompressed(DATA_Q))};
const DATA_H =\n  ${quote(writeCompressed(DATA_H))};`;

process.stdout.write(`${data}\n`);
process.stderr.write(`${data.length} bytes\n`);

function writeCompressed(entries) {
  // counterpart to readCompressed in corrections.mjs
  return entries
    .map(
      ({ g2, g1, g1s, ecs }) =>
        compressNum(g2, 1) +
        compressNum(g1, 1) +
        compressNum((g1s << 5) | ecs, 2),
    )
    .join('');
}
